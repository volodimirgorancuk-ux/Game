...
with open(base + 'obf_deobf/smart_mapping_v3.json', 'r') as f:
    smart = json.load(f)
...
for cls_file in class_files:
    ...
    for m in matches:
        ...
        if sig_key in smart:
            ...
            if sug != obf_name:
                renames.append((m.start(), obf_name, sug, m.group(0)[:50]))
    
    # Apply renames using string replacement to avoid position shifting
    content_lines = content.split('\n')
    rename_map = {old: sug for _, old, sug, _ in renames}
    
    new_lines = []
    for line in content_lines:
        new_line = line
        for old, sug in rename_map.items():
            # Replace method declarations: "old(" -> "sug("
            new_line = re.sub(r'\b' + re.escape(old) + r'\s*\(', sug + '(', new_line)
            # Replace qualified calls: "ClassName.old(" -> "ClassName.sug("
