...
def infer_name(method, caller_map, key_prefix):
    ...
    # STATE HANDLERS - only for EXACT matching signatures
    STATE_HANDLERS = {
        ('v', ''): 'updateMainMenu',
        ('x', ''): 'updateOptions',
        ('F', ''): 'updateLoading',
        ...
        ('dA', ''): 'updateVictory',
        ('eh', ''): 'updateCredits',
        ('fE', ''): 'updateLevelSelect',
        ('aO', ''): 'updateCharacterSelect',
        ('E', ''): 'updatePausedSubmenu',
        ('G', ''): 'updateConfirmDialog',
    }
    RENDER_HANDLERS = {
        ('b', 'Graphics graphics'): 'drawMainMenu',
        ('c', 'Graphics graphics'): 'drawOptions',
        ('h', 'Graphics graphics'): 'drawLoading',
        ('l', 'Graphics graphics'): 'drawCutscene',
        ('j', 'Graphics graphics'): 'drawInGame',
        ('e', 'Graphics graphics'): 'drawGameOver',
        ('P', 'Graphics graphics'): 'drawTutorial',
        ('R', 'Graphics graphics'): 'drawSubState',
        ('ar', 'Graphics graphics'): 'drawContinue',
        ('ao', 'Graphics graphics'): 'drawVictory',
        ('ax', 'Graphics graphics'): 'drawCredits',
        ('n', 'Graphics graphics, boolean bl'): 'drawLevelSelect',
        ('q', 'Graphics graphics'): 'drawCharacterSelect',
        ('g', 'Graphics graphics'): 'drawPausedSubmenu',
        ('i', 'Graphics graphics'): 'drawConfirmDialog',
    }
    
    state_key = (name, params[:50])
    if state_key in STATE_HANDLERS:
        return STATE_HANDLERS[state_key]
    if state_key in RENDER_HANDLERS:
        return RENDER_HANDLERS[state_key]
    
    # SPECIAL KEYS - exact name match
...
with open('/workspace/2e169882-ef71-42c0-8852-da4051cfb2af/sessions/agent_5f22a82b-5cc7-42ab-877e-bccd78a79285/obf_deobf/smart_mapping_v3.json', 'w') as f:
